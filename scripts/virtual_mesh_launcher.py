    ...
    while time.time() < deadline:
        ...
        if _check_ssh_reachable(host, port, timeout=2):
            if _run_ssh_check(host, port, timeout=15):
                return True
            logger.info("VM %s:%d: port open but SSH login not ready (attempt %d)", host, port, attempt)
        time.sleep(5)
    return False
...
def _setup_tap_mode(n_nodes: int) -> tuple[list[tuple[str, str]], Callable[[], None]]:
    ...
    for i in range(1, n_nodes + 1):
        ...
        _run_sudo(["ip", "tuntap", "add", "dev", tap, "mode", "tap", "user", os.environ.get("USER", "root")])
...
def launch_virtual_mesh(
    ...
    ssh_base = ssh_base_port or int(os.environ.get("VIRTUAL_MESH_SSH_BASE_PORT", str(DEFAULT_SSH_BASE_PORT)))
    timeout = boot_timeout or int(os.environ.get("VIRTUAL_MESH_BOOT_TIMEOUT", str(DEFAULT_BOOT_TIMEOUT)))
    ...
    if os.environ.get("VIRTUAL_MESH_SKIP_VWIFI", "").strip() == "1":
        vwifi_bin = None
    if not vwifi_bin:
        logger.warning("vwifi-server not found or skipped; VMs may not form mesh over WiFi")
        ...
        if vwifi_bin:
            ...
            for attempt in range(10):
                ...
                if "8214" not in result.stdout:
                    break
                logger.info("Waiting for port 8214 to be released (attempt %d)", attempt + 1)
                time.sleep(2)
            ...
            vwifi_cmd = ["sudo", "-n", vwifi_bin, "-u"] if use_sudo else [vwifi_bin, "-u"]
            ...
            if vwifi_proc.poll() is not None:
                log_content = vwifi_log.read_text() if vwifi_log.exists() else "(no log)"
        ...
        if use_tap:
            ...
            for i in range(1, n_nodes + 1):
                ...
                cmd = [
                    ...
                    "-nographic",
                    "-snapshot",
                    "-drive", f"file={image},if=virtio,format=raw,file.locking=off",
                    "-netdev", f"tap,id=net{i},ifname={tap_name},script=no,downscript=no",
                    "-device", f"virtio-net-pci,netdev=net{i},mac={mac}",
                ...
                if use_kvm:
                    ...
                    cmd.insert(2, "-cpu")
                    cmd.insert(3, "host")
                logger.info("Starting VM %d (TAP %s, expected IP %s)", i, tap_name, vm_ip)
                ...
                if _poll_ssh_ready(vm_ip, 22, timeout):
                    ...
                    if log_path.exists():
                        with open(log_path) as f:
                            tail = f.read()[-2000:]
                    logger.error("VM %d: SSH failed after %ds. Last log:\n%s", i, timeout, tail)
                    raise RuntimeError(f"VM {place_id}: SSH to {vm_ip} failed after {timeout}s")
        else:
            for i in range(1, n_nodes + 1):
                place_id = f"virtual-mesh-{i}"
                port = ssh_base + i - 1
                ...
                cmd = [
                    ...
                    "-snapshot",
                    "-drive", f"file={image},if=virtio,format=raw,file.locking=off",
                    "-device", f"virtio-net-pci,mac={mac},netdev=mesh0",
                    "-netdev", f"user,id=mesh0,net=10.13.0.0/16,hostfwd={hostfwd}",
                    "-device", "virtio-net-pci,netdev=wan0",
                    "-netdev", "user,id=wan0",
                    "-device", f"virtio-net-pci,mac=52:54:99:00:00:{i:02x},netdev=vwifi0",
            ...
            for i in range(1, n_nodes + 1):
                ...
                if _poll_ssh_ready("127.0.0.1", port, timeout):
                    nodes.append(VirtualMeshNode(
                        ...
                        host="127.0.0.1",
                        port=port,
                        ssh_args=["-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", "-p", str(port)],
                    ))
