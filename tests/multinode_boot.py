...
After booting to shell, assigns a unique test IP on br-lan via the serial
console, so multiple nodes on the same VLAN don't conflict (vanilla OpenWrt
...
def boot_node(
    ...
    logger.info("Node %s: test IP %s, SSH via %s (NetworkService: %s%%%s)",
                place, node_ip, vlan_iface, ns_ip, ns_iface)
