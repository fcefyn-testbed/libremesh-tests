...
**Switch / VLAN automation:** labs with isolated-per-DUT VLANs (hybrid mode) must install [labgrid-switch-abstraction](https://github.com/fcefyn-testbed/labgrid-switch-abstraction) **on the lab host**, with `switch-vlan` in PATH for the user that remote developers SSH as (typically a shared user like `labgrid-dev`, not the admin). Switch credentials should live in `/etc/switch.conf` (system-wide, group-readable) so any SSH user can run the command - see [fcefyn_testbed_utils switch-config: Multi-user setup](https://github.com/fcefyn-testbed/fcefyn_testbed_utils/blob/main/docs/configuracion/switch-config.md#multi-user-setup-recommended-for-labs-with-remote-devs). The test suite invokes `switch-vlan` remotely via `LG_PROXY` SSH; developers do not need the package, a `dut-config.yaml`, nor switch credentials locally.
